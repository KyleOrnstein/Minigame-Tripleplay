...
class KeysTestViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "gallows"{
            print("preformed")
            if let gvc = segue.destination as? GallowstestViewController{
                gvc.wrongs = wrongs
            }
        }
    }
